...
async function genentries(
    ...
    for (let i = 0; i < count; i++) {
        ...
        process.stdout.write(`[entry] ${entry.amount.toString()} ${entry.message} `);
        if (entry.category) process.stdout.write(`${entry.category.name} `);
        if (entry.tags) process.stdout.write(`${entry.tags.map(tag => tag.name).toString()} `);
...
async function gentags(user: User) {
    ...
    let tags: Tag[] = await Promise.all(
        tagNames.map(async (name) => {
            let tag = await prisma.tag.create({ data: { name, user_id: user.id } });
...
async function main() {
    ...
    await genentries(1000, user, categories, tags)
}
