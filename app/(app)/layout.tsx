...
function NavItemGroup({ text, children }: { text?: string, children: React.ReactNode }) {
    return (
        <div className="w-full flex flex-col px-4 pt-4">
            {text && <p className="py-1 px-4 text-sm text-muted-foreground">{text}</p>}
            {children}
        </div>
    )
}
...
function NavItem({ href, text }: { href: string, text: string }) {
    return (
        <Link className="w-full justify-start inline-flex items-center gap-1 px-4 py-2 rounded-sm hover:bg-accent" href={href}>
            {text}
