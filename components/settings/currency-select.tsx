        ...
        <div className="space-y-3">
            ...
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    ...
                    >
                        <span className={cn("truncate", !currency && "text-muted-foreground")}>
                            {currency
                                ? currenciesJson[currencyKeys.find(k => k === currency) as keyof typeof currenciesJson]?.name
                                : "Select currency"}
                        </span>
                        <ChevronDownIcon
                            size={16}
                        ...
                        <CommandList>
                            ...
                            <CommandGroup>
                                {currencyKeys.map((key) => (
                                    <CommandItem
                                        key={key}
                                        value={currenciesJson[key as keyof typeof currenciesJson].name}
                                        onSelect={() => {
                                            setCurrency(key)
                                            setOpen(false)
                                        }}
