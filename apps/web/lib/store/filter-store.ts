...
export const useFilterStore = create<FilterStore>()((set, get) => ({
	...
	filter_toggle: (field, operation, value, id) => {
		...
		if (idx < 0) return get().filter_add(field, operation, [value], id);
		...
		else values = [...values.slice(0, idx2), ...values.slice(idx2 + 1)];
		return get().filter_add(field, original_operation, [...values], id);
	},
	...
	filter_apply: q => {
		...
		for (const { field, operation, values } of get().filter_stack) {
			...
			if (!allowed_operations_map[field].includes(operation)) continue;
			...
			if (
				...
				if (values.length < 1) continue;
				const value =
					values[0].value instanceof Date ? values[0].value : null;
				...
				if (operation === "between") {
					const d1 = value;
					const d2 =
						values.length >= 2 && values[1].value instanceof Date ?
							values[1].value
						:	null;
