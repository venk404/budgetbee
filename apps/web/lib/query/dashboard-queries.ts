...
import { useFilterStore, serializeFilterStack } from "@/lib/store/filter-store";

export const useDashboardViews = () => {
	const { data: authData } = authClient.useSession();
	return useQuery({
		queryKey: ["dashboard", "list", authData?.user?.id],
		queryFn: async () => {
			...
			if (res.error) return [];
			return res.data as (DashboardView & { id: string; created_at: string; updated_at: string })[];
		},
...
export const useDashboardView = (id: string | undefined) => {
	...
	return useQuery({
		...
		queryFn: async () => {
			if (!id || !authData?.user?.id) return null;
			const db = await getDb();
			const res = await db
				.from("dashboard_views")
				.select("*")
		...
		enabled: !!id && !!authData?.user?.id,
	});
};

export type DashboardMutationProps =
	| { type: "create"; payload: { name: string; widgets?: WidgetConfig[] } }
	| { type: "update"; payload: { id: string; name?: string; widgets?: WidgetConfig[]; is_default?: boolean } }
...
export const useDashboardMutation = () => {
	...
	return useMutation({
		...
		mutationFn: async (data: DashboardMutationProps) => {
			...
			if (data.type === "create") {
				res = await db.from("dashboard_views").insert({
				...
				if (data.payload.name !== undefined) update.name = data.payload.name;
				if (data.payload.widgets !== undefined) update.widgets = data.payload.widgets;
				if (data.payload.is_default !== undefined) update.is_default = data.payload.is_default;
				res = await db
					.from("dashboard_views")
					.update(update)
		...
		onSuccess: (_, variables) => {
			...
			else if (variables.type === "update") toast.success("Dashboard saved");
			else if (variables.type === "delete") toast.success("Dashboard deleted");
...
export const useWidgetData = (widget: WidgetConfig | null) => {
	...
	return useQuery({
		queryKey: ["dashboard", "widget", widget?.id, widget?.dataSource, widget?.metric, widget?.aggregation, widget?.interval, filters],
		queryFn: async () => {
			...
			const res = await db.rpc("get_transaction_aggregate", {
				...
				p_organization_id: authData.session?.activeOrganizationId ?? null,
				p_filters: filters,
				p_metric,
				p_interval: widget.interval,
		...
		enabled: !!widget && !!authData?.user?.id,
	});
};
