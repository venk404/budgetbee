...
export function TransactionBarChart() {
    const { data } = useQuery({
        queryKey: ["tr", "agg"],
        queryFn: async () => {
            const res = await db.rpc("get_transaction_summary", { start_date: "2025-08-01", end_date: "2025-08-31" })
            console.log(res.data)
            if (res.error) {
                console.log(res.error)
            }
            console.log(res.data)
            return res.data.map((x: any) => ({ date: x.day, credit: x.credit, debit: x.debit }))
        },
    })
        ...
        <Card className="py-0">
            <CardHeader className="flex flex-col items-stretch border-b !p-0 sm:flex-row">
                <div className="flex flex-1 flex-col justify-center gap-1 px-6 pt-4 pb-3 sm:!py-0">
                    <CardTitle className="font-normal">Daily transactions</CardTitle>
                    <CardDescription>Showing all transactions for the past 30 days</CardDescription>
                </div>
                <div className="flex">
