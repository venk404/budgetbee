...
import { serializeFilterStack, useChartStore, useFilterStore } from "@/lib/store";
...
export function CategoriesPieChart() {
    ...
    const { data: chartData, isLoading: isTransactionsLoading } = useQuery({
        queryKey: ["chart", "by-cat", authData?.user.id, start, end, filterStack],
        queryFn: async () => {
    ...
    return (
        <Card className="py-0">
            ...
            <CardContent className="pb-6 max-sm:px-2">
                <ChartContainer config={chartConfig} className="relative aspect-auto h-[320px] w-full">
