...
import { useCategories, useTransactionDistributionByCategories } from "@/lib/query";
...
export function TransactionBarChart() {
    ...
    const { data: transactions, isLoading: isTransactionsLoading } = useTransactionDistributionByCategories();
    ...
    const chartData: Record<string, string | number>[] = React.useMemo(() => {
        ...
        const groupedTransactions: Record<string, Record<string, number>> = transactions.reduce(
            (acc, item) => {
                const key = format(new Date(item.day), "yyyy-MM-dd");
                if (!acc[key]) acc[key] = {};
                const name = catMap.get(item.category_id) || item.name || "Uncategorized";
                ...
                {isTransactionsLoading || isCategoriesLoading ?
                    ...
                    : <ChartContainer
                        ...
                        >
                            ...
                            {[...(categories || []), { name: "Uncategorized" }].map((c, i) => {
                                return (
                                    <Bar
                                        key={c.name}
                                        name={c.name}
